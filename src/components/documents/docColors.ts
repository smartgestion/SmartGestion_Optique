...
/**
 * Display labels for the prescription "traitement" (verre coating) values.
 * The form stores `aucun` for the "HC" option, so documents must map it back
 * to its human-readable label instead of printing the raw stored value.
 */
const TRAITEMENT_LABELS: Record<string, string> = {
  aucun: 'HC',
  'anti-reflet': 'Anti-reflet',
  'anti-lumiere-bleue': 'Anti-lumière bleue',
  photochromique: 'Photochromique',
  polarisant: 'Polarisant',
}

/** Map a stored verre traitement value to its display label (e.g. `aucun` → `HC`). */
export function formatTraitement(value: any): string {
  if (value === null || value === undefined || value === '') return ''
  const key = String(value).trim()
  return TRAITEMENT_LABELS[key] || key
}
