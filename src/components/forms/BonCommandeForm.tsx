...
export function BonCommandeForm({ initialData, onSuccess, readOnly = false }: BCFormProps) {
  ...
  // Targeted update of ONLY the cancellation reason on an already-cancelled
  // BC. This runs OUTSIDE react-hook-form's validation pipeline so it works
  // even when the (locked) verre/line fields would otherwise fail zod
  // validation and silently swallow the submit.
  const saveMotifOnly = async () => {
    if (!initialData?.id) return;
    setIsLoading(true);
    try {
      const motif = (form.getValues('motifAnnulation') || '').trim();
      const { error } = await supabase
        .from('bons_commande')
        .update({ motif_annulation: motif || null })
        .eq('id', initialData.id);
      if (error) throw error;
      toast.success(t('bons_commande.motif_saved', "Motif d'annulation enregistré"));
      onSuccess();
    } catch (err: any) {
      console.error('Motif update error:', err);
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
      ...
      <fieldset disabled={readOnly} className="space-y-8 border-0 p-0 m-0 disabled:opacity-100">
      ...
      <div className="flex justify-end items-center space-x-4 pt-6 border-t dark:border-white/5">
        ...
        {readOnly && isCancelled ? (
          // Locked BC: only the cancellation reason can be saved. Bypass RHF
          // validation (the locked verre/line fields would fail it) by saving
          // the motif directly via a plain button instead of a submit.
          <Button
            type="button"
