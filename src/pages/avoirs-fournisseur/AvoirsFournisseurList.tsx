...
import { Search, RotateCcw, Receipt, ChevronLeft, ChevronRight, Filter, Info, Plus, ArrowLeft, XCircle, ClipboardList, Lock, Download } from 'lucide-react'
...
import { AvoirFournisseurDocument } from '@/components/documents/AvoirFournisseurDocument'
...
interface AvoirFournisseur {
  ...
  montantTtc: number;
  statut: string;
  // 'simple' (Simple Avoir) or 'verre' (Verre Avoir) — mirrors the BC type.
  type: string;
  // 'manuel' or 'auto' (auto-created from a Bon de Commande cancellation).
  // Auto avoirs are read-only: history/preview/print only, no calculations.
  creationMode: string;
}
...
export function AvoirsFournisseurList() {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: selectedAvoir ? `Avoir_Fournisseur_${selectedAvoir.numero}` : 'Avoir_Fournisseur',
    onBeforePrint: async () => {
  ...
  const mapAvoir = (a: any): AvoirFournisseur => ({
    ...
    type: a.type || 'simple',
    // Fallback: a linked bon_commande_id implies an auto-created avoir even if
    // the column was not populated (legacy rows created before this feature).
    creationMode: a.creation_mode || (a.bon_commande_id ? 'auto' : 'manuel'),
  });
  ...
  const handleDownload = async (avoir: AvoirFournisseur) => {
    try {
      toast.info(t('shared.toast.pdf_preparing'));

      const { data: avoirData, error } = await supabase
        .from('avoirs_fournisseur')
        .select('*, bon_commande:bons_commande(*), fournisseur:fournisseurs(*)')
        .eq('id', avoir.id)
        .single();
      ...
      const { data: lignesData } = await supabase
        ...
        .select('*')
        .eq('avoir_fournisseur_id', avoir.id)
        .order('ordre');

      // For a verre avoir, pull the linked ordonnance so the optical layout
      // can render the OD/OG vision boxes + type de verre / indice / traitement.
      let prescription: any = null;
      if (avoirData.type === 'verre') {
      ...
      const mappedAvoir = {
        ...avoirData,
        type: avoirData.type || 'simple',
        numero: avoirData.numero,
        numeroFournisseur: avoirData.numero_fournisseur,
        bonCommande: avoirData.bon_commande,
        fournisseurId: avoirData.fournisseur_id,
        fournisseur: avoirData.fournisseur,
        prescription,
        // For auto-generated avoirs (created from a BC cancellation), the motif
        // is the BC's cancellation reason. Fall back to the linked BC's
        // motif_annulation since it may be entered after the avoir was created.
        motif: avoirData.motif || avoirData.bon_commande?.motif_annulation || null,
        notes: avoirData.notes,
        dateEmission: avoirData.date_emission,
        montantHt: avoirData.montant_ht,
        montantTva: avoirData.montant_tva,
        montantTtc: avoirData.montant_ttc,
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      // Auto-created avoirs are read-only and must never be cancelled here —
      // they never applied any calculation, so there is nothing to revert.
      if (target && target.creationMode === 'auto') {
        toast.error(t('avoirs_fournisseur.read_only_hint'));
        setCancelConfirmOpen(false);
        setAvoirToCancel(null);
        return;
      }
      // Only manual avoirs ever touched stock, so only they need a reversal.
      // A manual avoir reduces stock on create whether or not the user linked
      // a Bon de Commande for reference, so the BC link must NOT skip the
      // reversal. (AUTO avoirs, handled above, never moved stock.)
      if (target && target.creationMode === 'manuel') {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500">
      ...
      <div className="hidden">
        <AvoirFournisseurDocument ref={componentRef} avoir={selectedAvoir} entreprise={entreprise} lang={i18n.language} />
      </div>
                      ...
                      >
                        ...
                        <TableCell className="px-4 py-5">
                          <div className="flex flex-col items-start gap-1.5">
                            {/* Avoir type: Simple Avoir / Verre Avoir */}
                            <span className={cn(
                              "text-[11px] font-medium px-2 py-0.5 rounded-sm inline-flex items-center gap-1",
                              avoir.type === 'verre'
                                ? "dark:text-sky-400 dark:bg-sky-500/10 text-sky-600 bg-sky-50"
                                : "dark:text-slate-400 dark:bg-slate-500/10 text-slate-600 bg-slate-50"
                            )}>
                              {avoir.type === 'verre' ? t('avoirs_fournisseur.type_verre') : t('avoirs_fournisseur.type_simple')}
                            </span>
                            {avoir.creationMode === 'auto' ? (
                              <span dir="ltr" className="text-xs font-mono font-medium dark:text-emerald-400 dark:bg-emerald-500/10 text-emerald-600 bg-emerald-50/50 px-2 py-0.5 rounded-sm inline-flex items-center gap-1">
                                <ClipboardList className="h-3 w-3" />
                                {avoir.bonCommande?.numero || t('avoirs_fournisseur.auto_generated_badge')}
                              </span>
                            ) : (
                        ...
                        <TableCell className="px-4 py-5 text-start">
                          <div className="flex justify-end items-center gap-0.5">
                            ...
                            {avoir.creationMode === 'auto' ? (
                              // Auto-created avoirs are READ-ONLY: kept for history,
                              // preview and print only. No cancel/edit — the BC
                              // cancellation already handled all stock & expense
                              // calculations, so this avoir must not re-apply them.
                              <span
                                className="text-[11px] font-semibold px-2 py-1 rounded-sm inline-flex items-center gap-1 dark:text-emerald-400 dark:bg-emerald-500/10 dark:border-emerald-500/20 text-emerald-700 bg-emerald-50 border border-emerald-200/50"
                                title={t('avoirs_fournisseur.read_only_hint')}
                              >
                                <Lock className="h-3 w-3" />
                                {t('avoirs_fournisseur.auto_generated_badge')}
                              </span>
                            ) : (
                              avoir.statut !== 'annulé' && (
                                <Button
