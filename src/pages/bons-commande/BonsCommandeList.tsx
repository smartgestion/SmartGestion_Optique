...
export function BonsCommandeList() {
  ...
  // When a BC is opened for a non-brouillon status the form is view-only.
  ...
  const handleEdit = async (bon: BonCommande) => {
    try {
      ...
      // Only a "brouillon" BC stays editable. Every other status (envoyé,
      // confirmé, livré/livrée, annulé, refusé) opens in view-only mode.
      setFormReadOnly(bonData.statut !== 'brouillon');
          ...
          if (bcFull) {
            ...
            const { data: newAvf, error: avfError } = await supabase
              ...
              .insert([{
                ...
                // Avoir type mirrors the BC type: Verre Commande -> Verre Avoir,
                // Simple Commande -> Simple Avoir.
                type: bcFull.type === 'verre' ? 'verre' : 'simple',
                // Auto Created (Annulation de Bon de Commande). Read-only avoir kept
                // for history/preview/traceability only — NO stock/expense/totals
                // re-calculation (the BC cancellation already restored everything).
                creation_mode: 'auto',
                ...
                statut: 'annulé',
                // The avoir's motif is inherited from the BC cancellation reason.
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-foreground truncate">
                {editingBon ? (formReadOnly ? t('bons_commande.dialog_view', 'Consulter le bon de commande') : t('bons_commande.dialog_edit')) : t('bons_commande.dialog_create')}
              </h2>
                      ...
                      >
                        ...
                        <TableCell className="px-4 py-5 text-start">
                          <div className="flex justify-end gap-0.5">
                            ...
                            {['brouillon', 'envoyé', 'confirmé', 'livré', 'livrée', 'annulé'].includes(String(bon.statut)) && (
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-8 w-8 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-[4px] dark:hover:text-white dark:hover:bg-white/5 dark:rounded-sm"
                                onClick={() => handleEdit(bon)}
                                title={bon.statut === 'brouillon' ? t('shared.actions.edit') : t('shared.actions.view', 'Consulter')}
                              >
