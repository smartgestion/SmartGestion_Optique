...
export function Workspace() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data: bonsCommande } = await supabase.from('bons_commande').select('*').eq('user_id', user.id).in('statut', ['confirm\u00E9', 'livr\u00E9', 'livr\u00E9e']);
      ...
      // Map client id → full name so rendez-vous / prescription labels can fall
      // back to the client name when the embedded `clients(nom)` join returns
      // nothing (e.g. the local SQLite adapter) instead of showing a placeholder.
      const clientNameById = new Map<number, string>();
      for (const c of clients) {
        const name = (c as any).nom || (c as any).nom_societe;
        if (name) clientNameById.set((c as any).id, name);
      }
      ...
      const rdvs = (rdvRes.data || [])
        ...
        .map((r: any) => ({
          ...
          client_nom: r.clients?.nom || clientNameById.get(r.client_id) || r.client_nom || '',
      ...
      for (const p of (presRes.data || [])) {
        ...
        if (exp >= today && exp <= in60Days) {
          expiring.push({
            id: `pres-${p.id}`,
            kind: 'prescription',
            label: p.clients?.nom || clientNameById.get(p.client_id) || `#${p.id}`,
