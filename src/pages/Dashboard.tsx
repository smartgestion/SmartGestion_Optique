  ...
  const fetchDashboardStats = useCallback(() => {
    ...
    let bcQuery = supabase.from('bons_commande').select('*').eq('user_id', user.id)
      ...
      Promise.all([
        ...
        // Resolve each recent facture's client to its full name. The list UI
        // reads `facture.client?.nom`, but the query embeds it under `clients`
        // (and that join can return nothing on the local SQLite adapter), which
        // is why it used to fall back to "Client Passager". Look the name up by
        // `client_id` from the already-loaded clients so the full name shows.
        const recentClientNameById = new Map<string, string>()
        for (const c of clients as any[]) {
          const name = c?.nom || c?.nom_societe
          if (name) recentClientNameById.set(String(c.id), name)
        }
        for (const f of recentFacturesRaw as any[]) {
          const nom = f?.clients?.nom || f?.client?.nom || recentClientNameById.get(String(f.client_id)) || null
          if (nom) f.client = { ...(f.client || {}), nom }
        }
          ...
          <div className="grid gap-6 sm:gap-8 md:grid-cols-3">
            ...
            <div className="space-y-3">
              ...
              <div className="h-2 w-full bg-muted rounded-full overflow-hidden" dir="ltr">
                {(() => {
                  const collectee = Number(stats?.totalTvaCollectee ?? 0);
                  const deductible = Number(stats?.totalTvaDeductible ?? 0);
                  const max = Math.max(collectee, deductible);
                  const pct = max > 0 ? Math.min((collectee / max) * 100, 100) : 0;
                  return (
                    <div
            ...
            <div className="space-y-3">
              ...
              <div className="h-2 w-full bg-muted rounded-full overflow-hidden" dir="ltr">
                {(() => {
                  ...
                  const deductible = Number(stats?.totalTvaDeductible ?? 0);
                  const max = Math.max(collectee, deductible);
                  const pct = max > 0 ? Math.min((deductible / max) * 100, 100) : 0;
                  return (
                    <div
                      className="h-full bg-gradient-to-r from-[#0EA5E9] to-[#0EA5E9]/70 rounded-full transition-all"
                      style={{ width: `${pct}%` }}
                    />
                  );
                })()}
